...
public class EightPuzzleBoard {
	...
	public void swapWithGap(int n) {
		...
		if (gapPosition != n) {
			...
			int nxpos = xycoordinatesFromAbsoluteCoordinate(n)[0];
			int nypos = xycoordinatesFromAbsoluteCoordinate(n)[1];

			int value = getValueAt(nxpos, nypos);
			setValue(xpos, ypos, value);
			setValue(nxpos, nypos, 0);
		}
	}
