...
def mapper():
    ...
    for line in reader:
        ...
        count = 0
        for ch in line[4]:
            count += 1 #count the characters
            if ch == '.' or ch == '!' or ch == '?':
                if count != len(line[4]): #if they are not the last character
                    isOne = False         #then this is not one line  
                    break                 #break
                    
        #if isOne is still true, then print    
        if isOne == True:
